...
def collect_metadata(input_filename):
    ...
    while True:
        ...
        try:
            ...
            if 1 <= diff_value <= 100:
                metadata['difficulty'] = diff_value
                break
            else:
    ...
    # Optional: Genre
    genre = input("Genre [Electronic]: ").strip()
    metadata['genre'] = genre if genre else "Electronic"
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python main.py <input_audio_file> [--batch|--interactive]")
    ...
    try:
        ...
        if batch_mode or not is_interactive_environment():
            ...
            metadata = collect_metadata(input_filename)
